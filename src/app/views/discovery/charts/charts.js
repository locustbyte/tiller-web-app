...
angular.module('tillerWebApp')
    ...
    .controller('discovery.chartsCtrl', ['$scope', '$rootScope', 'userHistoryService', 'userService', 'restService', 'chart.assetAllocationService', 'PubSub', function ($scope, $rootScope, userHistoryService, userService, restService, chartAssetAllocationService, PubSub) {
         ...
         * Listen for the view.change event from the router to trigger view related settings
         */
        $rootScope.$on('profile.updated', function(e, state){
        ...
        /**
         * Setup the user service incase we are landing on this view
         */
        $scope.userDataSynced = false;
        userService.getUserDataAsync().then(function(response){
            $scope.userDataSynced = true;
        });
        ...
        $scope.getThemeControlState = function(level)
        {
            ...
            if(userService.getUserDataSync() !== undefined && userService.getUserDataSync().investorLevel === level)
            {
        ...
        $scope.switchInvestorLevel = function(level)
        {
            ...
            var user = userService.getUserDataSync();

            user.investorLevel = level;
        ...
        $scope.loadThemes = function() {
            ...
            restService.getPortfolioThemes($scope.state).then(function(themes) {
                ...
                $scope.$watch('themesList.items', function(newVal){
                    ...
                    else if($scope.userDataSynced && userService.getUserDataSync().investorLevel === 3)
                    {
                        $scope.resetThemeControlOverviewStates();
                        $scope.modals.addTheme = true;
                    }
         ...
         * @returns {*}
         */
        $scope.getInvestorLevelKeyFromUser = function()
        {
         ...
         * @returns {*}
         */
        $scope.getInvestorLevelKey = function(level)
        {
        ...
        $scope.maybeShowOverview = function()
        {
            ...
            var key = $scope.getInvestorLevelKeyFromUser();
